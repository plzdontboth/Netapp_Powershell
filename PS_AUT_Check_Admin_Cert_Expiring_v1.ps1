...
$hosts = @("<host1>,<host2>")  
$commands = @(
    "ro 0; security certificate show -cert-name admin.*; security certificate show -ca <Certificate_Authority>"
)

foreach ($hostname in $hosts) {
    $logFile = "C:\Users\<user>\Downloads\Pow_logs\PS_cert_output_$(Get-Date -Format 'yyyyMMdd_HHmm').txt"
